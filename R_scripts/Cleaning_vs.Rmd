...
```{r}
# Load in dataset
rank_info <- read.csv("C:/Users/Systeembeheer/OneDrive - Tilburg University/Documenten/Master/Marketing Analytics/Thesis/Thesis_R/rank_information_comp_vs6.csv", sep = ";", na.strings=c("", "NA"))
summary(rank_info)

# Load in dataset
video_info <- read.csv("C:/Users/Systeembeheer/OneDrive - Tilburg University/Documenten/Master/Marketing Analytics/Thesis/Thesis_R/video_information4.csv", sep = ";", na.strings=c("", "NA"))
summary(video_info)
...
# Cleaning new video_information dataset including all platforms
...
remove_col <- c("X", "F", "12", "AO", "Approved", "Passed", "13+", "16+", "T", "MA-17")
...
colnames(video_information_all_platforms)[47] <- "TV_13"
colnames(video_information_all_platforms)[48] <- "TV_Y"
colnames(video_information_all_platforms)[49] <- "TV_G"
colnames(video_information_all_platforms)[50] <- "TV_V7_FV"
colnames(video_information_all_platforms)[51] <- "TV_Y7"
colnames(video_information_all_platforms)[52] <- "NC_17"
```
...
video_information_all_platforms_cleaned <- video_information_all_platforms %>%
  mutate(G_General_audiences = ifelse(G == 1 | TV_Y == 1 | TV_G == 1, 1, 0),
         ...
         PG13_Parents_strongly_cautioned = ifelse(PG_13 == 1 | TV_14 == 1 | TV_13 == 1, 1, 0),
...
```{r}
#drop old columns
col_to_drop <- c("G", "TV_Y", "TV_G","PG", "TV_Y7","TV_V7_FV", "PG_13", "TV_14", "TV_13", "NC_17", "TV_MA", "Not_rated", "Unrated", "NA.y", "TV_PG")
...
```{r}
all_platforms_cleaned <- video_information_all_platforms_cleaned %>%
  select(!rank) %>%
  select(!exclusive) %>%
...
all_platforms_cleaned$platform[all_platforms_cleaned$platform == 'Amazon'] <- 'Amazon Prime'

write.csv(all_platforms_cleaned, "all_platforms_cleaned.csv")
```
...
netflix_data_wide <- video_information_all_platforms_cleaned %>%
  filter(platform == "Netflix")
head(netflix_data_wide)

# creating data set for competitor videos. 
competitor_data_wide <- video_information_all_platforms_cleaned %>%
  filter(platform != "Netflix")
...
num_videos_week_comp <- competitor_long %>%
  group_by(week_num, genre_mpaa) %>%
  count(genre_mpaa)

head(num_videos_week_comp)
...
```{r}
netflix_merged_comp <- netflix_long %>%
  distinct() %>%
  left_join(num_videos_week_comp, by = c("week_num", "genre_mpaa")) %>%
...
AVG_number_same_genre <- netflix_merged_comp %>%
  filter(str_detect(genre_mpaa, "genre_")) %>%
  group_by(title.name, week_num) %>%
  summarise(AVG_Ngenre_comp = sum(n)) %>%
  group_by(title.name) %>%
...
netflix_merged_wider <- netflix_merged_without_week %>%
  pivot_wider(names_from = genre_mpaa, values_from = value)
```
...
netflix_merged_wider <- netflix_merged_wider %>%
  select(!genre_Drama:genre_Western)
```
...
*The sequel will be added by hand because this can be not found on IMDB or Flixpatrol. So I have to export the dataset to manually add this column*
```{r}
write.csv(netflix_merged_wider, "netflix_merged_wider.csv", row.names = FALSE)
```
...
```{r}
write.csv(netflix_data_complete, "netflix_data_complete.csv")
```
