...
```{r}
# Load in data set
all_platforms_cleaned <- read.csv("C:/Users/Systeembeheer/OneDrive - Tilburg University/Documenten/Master/Marketing Analytics/Thesis/Thesis_R/all_platforms_cleaned.csv", sep = ",", na.strings=c("", "NA"))
summary(all_platforms_cleaned)

# Load in data set
netflix_data_complete <- read.csv("C:/Users/Systeembeheer/OneDrive - Tilburg University/Documenten/Master/Marketing Analytics/Thesis/Thesis_R/netflix_data_complete.csv", sep = ",", na.strings=c("", "NA"))
...
```{r}
#removing NA
all_platforms_cleaned <- na.omit(all_platforms_cleaned)
```


# number of distinct videos for each platform
```{r}
# Create a table of average ratings by platform
avg_ratings <- all_platforms_cleaned %>% 
  group_by(platform) %>% 
  summarize(count = n()) %>% 
  arrange(count)
```
...
platforms_wider_count <- avg_ratings %>%
  pivot_wider(names_from = platform, values_from = count)

empty_col <- ""
platforms_wider_count <- cbind(empty_col, platforms_wider_count)
colnames(platforms_wider_count)[1] <- "genre_mpaa"
```
...
```{r}
# join the two data frames together
all_platforms_genre_mpaa <- rbind(platforms_wider_count, platforms_wider_genre, platforms_wider_mpaa)
```
